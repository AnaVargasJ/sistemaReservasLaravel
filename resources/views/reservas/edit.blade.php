...
@section('titulo', 'Editar reservacion')

@section('content')
    <h2 class="texto-blanco pt-5 pb-3"id="text">Editar Reservacion</h2>
    @if ($errors->any())
    ...
    @endif
    <form class="my-3" action="{{ route('reservas.update', $reserva->id) }}" method="post">
        @method('PUT')
        @csrf
        <div class="card mt-4">
        ...
        <div class="pt-5 pb-3">
            <button type="submit" class="btn btn-info mb-4" onclick="return confirm('¿Desea agregar la silla?')">
                Guardar
            </button>
        </div>
    </form>
